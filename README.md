...
There is a master seed for each file that helps prevent key reuse - master keys for each file are derived from password and master seed. Master seeds are stored both inside the encrypted file and in separate files (a little hackish).  
